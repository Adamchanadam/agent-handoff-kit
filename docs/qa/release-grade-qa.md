...
- 產品範圍：開工只用最少交接事實安全設定目前 thread 的顯示名稱；名稱不會成為工作真源或權限。收工檢查改以明確的肯定斷言及相同範圍比對，保留否定、限制、待處理、時間和環境條件；同範圍真矛盾仍拒絕。升級只會替換來源目錄精確證明的完整舊官方 opening 前綴，保留其後的用戶內容；若來源的兩個唯一未變行仍包圍遺失的現行官方行，只會補回該行，不刪除中間本地文字。舊版已代入本機根目錄的首行亦只在完整官方前綴吻合時處理。候選預檢會在凍結前綁定目前 runtime、情景及 QA 矩陣，並檢查已改雙語公開頁的獨立 hash-bound PASS 證據及目前 Cross-mind 表，避免把這類契約或文件缺口延遲到 full gate 才發現。
- 回歸範圍：開工命名 cases、closeout card contract、pack scenarios、歷史 v0.1.0 官方開場／官方前綴加自訂續行內容、現行安全行遺失但本地文字仍保留，以及 v0.3.64 custom opening message 的升級、closeout、repeat no-op；runtime-content upgrade safety 均已在候選前的來源工作中通過；本候選仍須在 commit 凍結、獨立唯讀覆核與正式 full 中重新綁定。
...
|---|---|---|---|
| 1. Failure or blocker | yes | iterated | The first full gate exposed missing bilingual evidence and the second exposed this required table; both were moved into pre-freeze checks. Later full replays exposed both the v0.1.0 official multi-line opening path and a missing current official safety line; catalog-proven prefix and bounded missing-line repairs now retain official/custom-suffix and local-text counterexamples before re-freeze. Candidate-preflight now binds the 4f source-derived guard from current runtime through its scenario and QA matrix before the final candidate is frozen. |
...
- 產品範圍：按官方完整內容證明升級核心／規則，保留自訂前後內容及 BOM／換行；未知核心或規則停止且零寫入。修正真實 log entry 與範本的邊界。既有交接範本保存父子任務、續接點、未完驗收、來源版本與閱讀深度；收工必須重建核對並記錄證據，CLI 拒絕明示不足或假欄位，但不認證語意真實。
- 舊安裝影響：升級保留既有使用者交接內容，不捏造父目標或補寫重建成功；下一次明確收工才按原要求／來源補足必要事實。完整保存的交接可如實描述未完成或受阻工作。
- 測試範圍：官方歷史來源目錄新增真正已發布 v0.3.64；永久機械 replay 及五組虛構 writer/reader 案例已入現有 QA 路徑。一次複雜舊安裝 → 升級 → 無歷史寫包者收工 → 另一無歷史讀者開工演練保留四層任務、三分支及八來源；開工前後 40 檔內容相同。此證據不代表任何模型或領域均無誤。

### pre-release final audit（v0.3.65，SOURCE_REVIEW_ACCEPTED）

- Clean artifact boundary：完成修補、公眾文件、獨立雙語審閱及目前 QA 報告後，才以已授權本地 commit 固定候選。實際候選證據須綁定 clean HEAD、package version、tarball SHA-256、manifest digest、release-readiness inventory digest、review bundle SHA-256、review subject digest 及本報告 hash。
- full 必須等 clean commit、凍結產物、accepted independent review receipt 及 five-conclusion writer assessment，不能把 dirty-tree scoped PASS 當正式 full。
- Full-check role isolation：writer 為本次產品修補者；正式 reviewer 必須是獨立唯讀角色。凍結 reviewSubject / review bundle history 保持以 `WAITING_INDEPENDENT_REVIEW` 結束；接受後只在候選 evidence history 加入 `REVIEW_ACCEPTED`。
- five-conclusion writer assessment 涵蓋 governanceHealth、productJourney、userJourney、qcBackflow、rulesPacksRouting；必須以目前實際來源／演練／指令結果逐項判斷，不能從下方歷史記錄推定。
- 已重現並修正獨立審查反例：前期安裝修改核心指令後被覆寫；同名未標記核心可能丟失本地規則；未知 safety 缺句被舊 anchor 修補器改回；行尾註解使否定回答消失；縮排程式碼被當證據。四種舊自訂核心 × init/upgrade、三類未知 pack 缺句的零寫入回歸及收工卡正反例通過；核心／可見欄位修補的獨立唯讀復核接受。這不是正式凍結候選審閱收據。
- 本輪 scoped 結果：完整 upgrade-safety（78 個官方歷史版本、保留／衝突／中斷／恢復／重跑）、closeout-card、closeout-efficiency、pack-scenarios、startup-status-only、真 v0.3.64 lifecycle replay、quick、v0.3.65 candidate-preflight，以及實際 4c/4d 情境通過。QA assurance 首次因缺本候選報告而停止；報告及全部雙語證據完成後重跑通過，涵蓋 runner 終態、必跑檢查失敗傳遞、preflight 正反例及近似有效但不合契約的 full/postpublish evidence 拒絕。
- 四組中英公眾文件已由獨立唯讀 reviewer 接受，來源／目標 hash 如下；4c/4d 實際情境與現行 QA 表已同步。待完成：取得本地候選 commit 授權、凍結獨立審閱及正式 full。無 push、tag、GitHub Release、npm publish 或 postpublish 聲稱。

### Cross-mind evidence 9-trigger table（v0.3.65）

| Trigger | Applies | Status | Notes |
|---|---|---|---|
| 1. Failure or blocker | yes | iterated | Reproduced stale runtime content after upgrade and missing-parent/source-depth false-complete; independent review added pre-formal unknown-core and Markdown visibility counterexamples, repaired at the existing owners. |
| 2. External side effects | yes | passed | Candidate preparation and isolated tests only; no publication or external delivery performed or authorized by this record. |
| 3. User-visible output | yes | iterated | Handoff identifies the parent, next bounded action and evidence gaps; CLI saved state does not certify underlying task completion. |
| 4. Complexity or boundary | yes | passed | Existing core, closeout pack, handoff fields and CLI checks own the change; no second task framework or automatic LLM semantic validator. |
| 5. Documentation drift | yes | passed | All four changed bilingual pairs were read fully and accepted after source-first repairs; actual 4c/4d scenario behavior and the current QA table now agree on ownership and zero-write conflict. |
| 6. Semantic runtime effect | yes | passed | An independent writer and reader preserved the complex task and safe startup boundary; a source blocker remains explicit rather than fabricated away. |
| 7. Cross-agent / role boundary | yes | iterated | History-free fixture agents and independent read-only code/bilingual reviewers were separate from the implementer; formal frozen-candidate review remains required. |
| 8. Real user journey | yes | passed | Genuine published init, local user handoff, upgrade, authorized closeout, fresh bare startup and unchanged startup file hashes were checked; underlying fictional task was not executed. |
| 9. Release statement | yes | passed | Local v0.3.65 candidate only; published v0.3.64 remains authoritative until an explicitly authorized release and postpublish readback. |

### Bilingual README semantic gate（v0.3.65，PASS）
...
- 產品範圍：runtime 明確 `SESSION_HANDOFF` 是已保存接力快照，任務進行中、兩次 Persistence Gate 判斷之間可合理落後於已讀回的任務真源；同時加入 `workspace-health` / `closeout-status` worktree truth guard，並修補 QA temporary fixture cleanup。
- release surface 範圍：package/runtime Stack/README/HTML/CHANGELOG/whatsnew/release QA 與 official-origin latest published lineage 對齊 v0.3.62 candidate；v0.3.61 保留為最新已發布 lineage，v0.3.62 不進 official-origin published catalog。
- full / release 邊界：本段記錄 candidate source state、expected-lag / workspace-health / temp-cleanup QA backflow 與 release QA readback material；不是 formal full PASS、release authorization、npm publish 或 postpublish evidence。

### pre-release final audit（v0.3.62，PUBLIC_SURFACE_REVIEW_ACCEPTED）

- Clean artifact boundary：v0.3.62 clean candidate commit and tarball must be created from the source tree after the accepted workspace-health closeout guard, QA temporary cleanup, handoff expected-lag correction, official-origin v0.3.61 published fixture, bilingual public-surface review, and required release surfaces are synchronized. The final candidate evidence must bind the resulting clean HEAD, package version, tarball SHA-256, manifest digest, release-readiness inventory digest, review bundle SHA-256, review subject digest, and release QA hash.
- full 必須等 clean commit, frozen tarball SHA-256, manifest digest, release-readiness inventory digest, review bundle SHA-256, review subject digest, accepted independent review receipt, and five-conclusion writer assessment before PASS.
- Full-check role isolation keeps the frozen reviewSubject / review bundle history ending `WAITING_INDEPENDENT_REVIEW`; final accepted candidate evidence later changes only the candidate evidence state history to end `REVIEW_ACCEPTED` and supplies the independent review receipt.
- five-conclusion writer assessment must cover governanceHealth、productJourney、userJourney、qcBackflow、rulesPacksRouting. For this candidate, `qcBackflow` is not passed unless workspace-health blocks false handoff Workspace Identity, QA temp roots are cleaned on PASS, and expected-lag wording cannot suppress required checkpoint / closeout cases.
- 發布打包邊界：本段是 v0.3.62 發布前候選證據；正式發布狀態以 GitHub Release、npm `@latest`、remote tag 和 postpublish readback 為準。

### Cross-mind evidence 9-trigger table（v0.3.62）

| Trigger | Applies | Status | Notes |
|---|---|---|---|
| 1. Failure or blocker | yes | passed | Real runtime feedback showed AI could treat a legitimately stale `SESSION_HANDOFF` snapshot as drift and over-update handoff during ordinary work; the candidate defines expected lag without weakening checkpoint / closeout requirements. |
| 2. External side effects | yes | iterated | No v0.3.62 push, tag, GitHub Release, npm publish, or postpublish readback is claimed here; those remain release-sequence gates after formal full. |
| 3. User-visible output | yes | passed | Users should see less handoff churn during ongoing tasks while still getting honest blocked closeout when workspace identity, prompt mirror, persistence, or doctor readback is not healthy. |
| 4. Complexity or boundary | yes | passed | The fix stays in existing owners: core Persistence Gate owns task checkpoint semantics, closeout-status owns semantic closeout readback, and release-readiness owns regression checks. No new pack, mode, or parallel governance layer is added. |
| 5. Documentation drift | yes | iterated | Source package surfaces, README, HTML pages, CHANGELOG, whatsnew, release QA, and official-origin latest published lineage are synchronized to v0.3.62 candidate state. |
| 6. Semantic runtime effect | yes | passed | Expected-lag wording is paired with explicit exceptions for durable / startup-needed facts, smallest-correct-home handoff updates, Kit-managed / release / closeout / external-effect state, and full closeout. |
| 7. Cross-agent / role boundary | yes | iterated | Independent bilingual review accepted the public version-token update; sub-agent反審 also caught the original too-broad wording before implementation. |
| 8. Real user journey | yes | passed | A user doing ordinary task work should not pay a handoff write after every intermediate step; a user saying「收工」or creating non-rebuildable durable state still gets persistence through the gate. |
| 9. Release statement | yes | iterated | v0.3.62 remains an unpublished source candidate until formal full, push, tag, GitHub Release, npm publish, and postpublish readback succeed. |

### Bilingual README semantic gate（v0.3.62，PASS）

- Reviewer：agent `01a02f02-5b1d-7e72-9bf8-ea937065f8f8`（Laplace）；provenance `codex-readonly-v0362-bilingual-public-pairs-20260823`；independent read-only scoped review of the changed public bilingual pair；verdict `accepted`.
- Scope：v0.3.62 candidate changed this pair only by source package version token relative to the accepted v0.3.61 baseline; public usage flow and meaning are unchanged.
- `README.md` SHA-256 `8F1907F5FBE467BAF70B9E06914682690472C29C0E831CCC4E63351328CF4767`
- `README.en.md` SHA-256 `1348BBF91974CD622D30D7E6A68E6FAADD712B5934DF4E0606FD9E17366352A3`
- Verdict: **PASS** — English preserves the same public user journey, safety boundaries, version-source boundaries, and install / upgrade / doctor / closeout command meanings; only `v0.3.61` became `v0.3.62`.

### Bilingual practical-guide semantic gate（v0.3.62，PASS）

- Reviewer：agent `01a02f02-5b1d-7e72-9bf8-ea937065f8f8`（Laplace）；provenance `codex-readonly-v0362-bilingual-public-pairs-20260823`；independent read-only scoped review of the changed public bilingual pair；verdict `accepted`.
- Scope：v0.3.62 candidate changed this pair only by visible version tokens and sample CLI version output relative to the accepted v0.3.61 baseline; install / upgrade / doctor / closeout / release meaning is unchanged.
- `agent-handoff-kit-guide.html` SHA-256 `81B97EF2A889D62E237AA65A38BA09FDB11D82C7F7F357A21218090598A9EB60`
- `agent-handoff-kit-guide.en.html` SHA-256 `05EE5337FFFEFEEEAC8C5461DADE4F1139E91DEA25EB2CADDD2316D8C26F79C1`
- Verdict: **PASS** — English preserves the same three public journeys, install-to-handoff flow, closeout meaning, and external publish authorization boundary; only version tokens / sample version output changed.

### Bilingual AI-install semantic gate（v0.3.62，PASS）

- Reviewer：agent `01a02f02-5b1d-7e72-9bf8-ea937065f8f8`（Laplace）；provenance `codex-readonly-v0362-bilingual-public-pairs-20260823`；independent read-only scoped review of the changed public bilingual pair；verdict `accepted`.
- Scope：v0.3.62 candidate changed this pair only by AI install page version tokens / version-note synchronization relative to the accepted v0.3.61 baseline; safety boundary and flow semantics are unchanged.
- `agent-handoff-kit-ai-install.html` SHA-256 `3934E968785B880036C3B81409088F7466289879AFFE88ACDB80BF656D7D15B4`
- `agent-handoff-kit-ai-install.en.html` SHA-256 `70B0E3DDB37E42F171B59825F4DA25DF1B2D874459DFD1F9666BE313DFC98381`
- Verdict: **PASS** — English preserves the same AI install / upgrade decision path, folder confirmation first, zero-write conflict boundary, `upgrade --dry-run`, `upgrade --yes`, `doctor`, and completion meaning; only version tokens changed.

### Bilingual introduction semantic gate（v0.3.62，PASS）

- Reviewer：agent `01a02f02-5b1d-7e72-9bf8-ea937065f8f8`（Laplace）；provenance `codex-readonly-v0362-bilingual-public-pairs-20260823`；independent read-only scoped review of the changed public bilingual pair；verdict `accepted`.
- Scope：v0.3.62 candidate changed this pair only by hero version note and footer version token relative to the accepted v0.3.61 baseline; beginner introduction meaning is unchanged.
- `agent-handoff-kit-intro.html` SHA-256 `9FE8D273487487BE451E064711009B787E3C0CC9F2BB9BB7378FE0C467094252`
- `agent-handoff-kit-intro.en.html` SHA-256 `E5025C538AED019208B444F303EF36A58D89731866463BA99650292F97CAD97D`
- Verdict: **PASS** — English preserves the same beginner introduction journey, supported-runtime boundary, startup / closeout meaning, safety boundaries, and npm registry version boundary; only version tokens changed.
...
| GitHub Release body 固定結構驗收 | 已併入 `npm run qa:release` 與發佈後 `gh release view` 回讀 | GitHub Release title 必須使用 `vX.Y.Z - <用戶可理解的價值短句>`；body 必須以 `# vX.Y.Z` 開首，並依序使用 `## 本版新加了甚麼`、`## 對你已有檔案的影響`、`## 建議下一步` 三段。若該版本有補充圖，唯一允許位置是 `# vX.Y.Z` 後、第一個 H2 前的一張版本圖；不得把圖解展示規則、維護策略或索引安排寫入 body。body 應由 `docs/whatsnew/vX.Y.Z.md` 生成或保持等價；不得回退成舊 `## 用戶價值` 格式。建立或修改 GitHub Release 前，若使用臨時 notes file 或非 `docs/whatsnew/vX.Y.Z.md` 的實際發佈稿，必須設定 `AGENT_HANDOFF_KIT_RELEASE_TITLE` 與 `AGENT_HANDOFF_KIT_RELEASE_NOTES_FILE` 跑 `npm run qa:release`，讓 gate 驗證實際送到 `gh release create/edit` 的 title / body；建立或修改 GitHub Release 後，必須用 `gh release view vX.Y.Z --json name,body` 回讀核對。 | 是 |
...
| First task → closeout → next session handoff | 收工後下一個 AI 能否從交接重建父目標／consumer、父子任務、續接點、未完驗收及各真源閱讀深度；不重開已完成子項，不把子項或交接完成當整體成果完成；handoff 與便利副本同源。 | `closeout-status` 正反例 + `check-handoff-continuity.mjs` 真舊版流程 + `test-fixtures/continuity/README.md` 獨立無歷史 writer/reader 與隱藏 rubric + prompt mirror readback；機械 PASS 不代替語意驗收 | 阻擋 publish，直到原要求／真源對照及實際開工邊界通過；保留首次失敗，不補造證據 |
...
| Existing project upgrade → proven content migration or unknown-content conflict | 完整官方核心／rules pack 正文或有界官方片段經來源證明後，才可更新並保留自訂周邊位元組；同名標題、同號規則、標記及缺 anchor 均不授權替换未知內容。未知 marked/unmarked/sandwich core、`dev/rules/safety.md` 自訂句、`dev/rules/integrations.md` 或 `dev/rules/onboarding.md` 未證正文須停止零寫入。`SESSION_HANDOFF.md`、`SESSION_LOG.md`、`PROJECT_DECISIONS.md` 等現有可變狀態只按其專屬受限策略遷移，不捏造重建成功或覆寫歷史。doctor 的 anchor 判定與 upgrade 所需來源證明是不同責任，不能靠把裸 anchor 放回檔尾取得假綠。 | Scenario 4c / 4d / 4e automated + upgrade quality matrix + 官方歷史 artifact 正例 + 前期無 formal guard 的自訂 core 反例 + 缺句 safety/onboarding/governance 反例 + misplaced-anchor / misplaced-handoff / fake-project-index / repair-marker / unsafe-safety-custom / unsafe-integrations / unsafe-onboarding / session-log-preserve | 阻擋 publish，直到官方內容正常更新且未知內容完整保留；衝突由具檔案能力的 AI 在用戶明確授權下作語意合併，不把判斷責任交给新手 |
...
| 4c | upgrade metadata migration with stale prompt convenience copy（current managed core 與 USER_RULES 保持一致，附本地前言；只讓 Stack 版本落後以觸發合法遷移） | `START_NEXT_SESSION_PROMPT.txt` 與 authoritative handoff opening 全文一致；本地 AGENTS/USER_RULES/handoff/log 位元組保留；migration committed / project health: passed | status: failed / 未知 core 被覆寫 / 遷移後便利副本仍落後 |
| 4d | upgrade unknown safety edit conflict（`dev/rules/safety.md` 的 `cmd /c rmdir` 被未知文字改動） | doctor 顯示 anchor checks failed 與 dev/rules/safety.md；upgrade conflict；zero writes；no lock | migration committed / restore safety pack high-risk rules in ## Rules section / 未知本地規則被覆寫 |
...
| 4f | upgrade no-op schema auto-repair（已 latest 零改動，但 opening message 缺 root mismatch guard） | `restore root mismatch guard in Next Session Opening Message` / `handoff opening message structure` / `If the root does not match the handoff` / `status: passed` /「升級驗收完成」 | 「完整 doctor 健康檢查未通過」/ `status: failed` /「你已經是最新版本」成功句 /「繼續日常使用即可」/「安裝完成」/「升級完成」 |
