...
狀態：原始碼候選版本，尚未發布。正式發布狀態以 GitHub Release 與 npm `@latest` 發布後讀回為準。

- 開工時會在最小必要狀態讀取後，於回覆狀態卡或開始同句已授權工作前，安全地把新 thread 改成「專案｜當前行動」。已是明確而相關或由用戶指定的名稱會保留；沒有可靠專案名稱便跳過，不會為改名額外讀取專案內容。
- 補回 title checkpoint 的讀回與失敗邊界：沒有安全能力時不阻塞開工；工具回覆含糊時只作一次針對性讀回及更正，不作背景重試、無關 thread 掃描或任務管理。
- 收工狀態檢查改為比較「肯定完成／足夠」與其同一斷言範圍，而非把同一段落的簡單關鍵字視作衝突。否定、有限、待處理或不同環境／時間條件會保留原意；真實同範圍矛盾仍會阻擋收工。
- 升級交接檔時，會替換來源目錄精確證明的完整舊官方 opening 前綴（包括舊版代入本機根目錄的首行）；若兩個唯一、未變的現行官方行之間遺失一行，會補回該來源行而不刪除中間的本地文字。用戶在其後加上的內容及鄰近文字保持原樣，避免升級把真實接力內容覆寫成空白範本，也讓早期正式版本可安全進入現行開工契約。
- 加入開工命名、收工斷言範圍、歷史安裝升級與重複升級的回歸檢查；候選預檢亦會把目前 runtime、升級情景與 QA 矩陣的契約一致性提早鎖住，並把已發佈 v0.3.66 納入官方來源目錄，供下一次安全升級辨識使用。

## v0.3.66 — 2026-09-05

狀態：原始碼候選版本，尚未發布。正式發布狀態以 GitHub Release 與 npm `@latest` 發布後讀回為準。

- 補強治理寫入邊界：新增或修改規則前先讀既有分類、規則包、專案補充及必要引用；保護完整官方正文，專案規則合併到既有可寫位置，減少日後升級衝突。
- 核心要求按任務讀取並運用適用規則及補充，遵守用戶目標與指令優先級，不自行另定目標、驗收門檻或替代規格。
- 安裝／升級成功後，AI 可採用同次完整健康結果，正常流程免再跑一次 doctor；失敗、中斷、輸出不足、後續相關改動或用戶另行要求檢查，仍須恢復或重驗。無需更新的升級明示同次健康結果並保留提醒；最新版本仍以 registry 核對為準。
- 官方檔案旁的本地補充在修改前後核對保護區及升級相容性；普通狀態資料仍用局部檢查。已混入正文的舊規則須核對來源後整理，不自動猜刪或以重裝繞過。
- 加入真實 v0.3.64／v0.3.65 安裝的規則新增、修改、升級及重複升級回歸，涵蓋 LF／CRLF；預演及衝突拒絕檢查完整目錄零寫入。既有流程加入獨立寫入者至無對話歷史讀者的運用演練。
- 官方來源目錄加入已發布 v0.3.65；來源檢查容許尚未提升版本號的開發來源收錄同版已發布基線，候選預檢仍要求候選版本高於 npm 最新正式版。

## v0.3.65 — 2026-09-05

狀態：原始碼候選版本，尚未發布。正式發布狀態以 GitHub Release 與 npm `@latest` 發布後讀回為準。

- 升級判定核對完整官方規則內容，修正換行格式、局部自訂內容及先前只更新版本號造成的舊規則遺留；保留可安全辨識範圍外的本地內容，未知修改仍停止處理。
- 修正工作日誌實際條目被誤認為範本標記而阻擋升級的情況；升級結果與整個項目的健康狀態分開報告。
- 既有交接範本與收工規則補強整體目標、子任務、準確續接點、剩餘驗收，以及真源版本、閱讀範圍、深度與缺口；簡單任務保持精簡。
- 收工前要求只憑交接內容重建任務，再對照原始要求及實際成果。收工檢查拒絕明示不充分、未知、缺漏、重複或缺證據的充分性紀錄；清楚保存阻礙與續接條件的未完成工作仍可交接。
- 加入升級內容、舊安裝收工至開工、交接缺漏與簡單任務的回歸案例。狀態卡反映保存狀態與已記錄的檢查，不自動證明任務結論正確。
